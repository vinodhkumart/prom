...
#########################################################################################
GRAFANA_DEFAULT_VERSION=6.1.4
PROMETHEUS_DEFAULT_VERSION=v2.9.1
PROMETHEUS_OPERATOR_DEFAULT_VERSION=v0.29.0
PROMETHEUS_STORAGE_DEFAULT_RETENTION=168h
PROMETHEUS_STORAGE_DEFAULT_VOLUME_SIZE=50Gi
...
NODE_LABEL_DEFAULT_KEY=node-type
NODE_LABEL_DEFAULT_VALUE=Infrastructure
DEFAULT_NAMESPACE=n
DEFAULT_CLUSTERNAME=func1
...
#Get Cluster name
read -p "Enter cluster name [$DEFAULT_CLUSTERNAME]: " CLUSTERNAME
CLUSTERNAME=${CLUSTERNAME:-$DEFAULT_CLUSTERNAME}
tput sgr0
...
options=("AWS" "GCP" "Azure" "Docker4Mac" "Vmware" "Custom")
...
do
    ...
    case $opt in
        ...
        "Azure")
            ...
            ;;
        "Docker4Mac")
            echo "Deploying on Docker4Mac"
            STORAGE_CLASS_PROVISIONER=no-provisioner
            STORAGE_CLASS_TYPE=localdisk
            echo -e "${BLUE}Are you deploying in Docker4Mac?"
            tput sgr0
            read -p "Y/N [N]: " use_aks
            if [[ $use_aks =~ ^([yY][eE][sS]|[yY])$ ]]; then
              echo "Remove rules for self hosted clusters"
            ...
            sed -i -e 's,STORAGE_CLASS_PROVISIONER,'"$STORAGE_CLASS_PROVISIONER"',g' manifests/prometheus/prometheus-k8s.yaml;
            sed -i -e 's,STORAGE_CLASS_TYPE,'"$STORAGE_CLASS_TYPE"',g' manifests/prometheus/prometheus-k8s.yaml;
            sed -i -e 's,STORAGE_CLASS_TYPE,'"$STORAGE_CLASS_TYPE"',g' manifests/prometheus/self-hosted/persistent-volume.yaml;
            sed -i -e 's,STORAGE_CLASS_TYPE,'"$STORAGE_CLASS_TYPE"',g' manifests/grafana/grafana.pvc.yaml;
            sed -i -e 's,STORAGE_CLASS_TYPE,'"$STORAGE_CLASS_TYPE"',g' manifests/grafana/self-hosted/persistent-volume.yaml;
            break
            ;;
        "Vmware")
            echo "Deploying on Vmware"
            STORAGE_CLASS_PROVISIONER=kubernetes.io/vsphere-volume
            STORAGE_CLASS_TYPE=gp2
            sed -i -e 's,STORAGE_CLASS_PROVISIONER,'"$STORAGE_CLASS_PROVISIONER"',g' manifests/prometheus/prometheus-k8s.yaml;
            sed -i -e 's,STORAGE_CLASS_TYPE,'"$STORAGE_CLASS_TYPE"',g' manifests/prometheus/prometheus-k8s.yaml;
            sed -i -e 's,STORAGE_CLASS_TYPE,'"$STORAGE_CLASS_TYPE"',g' manifests/grafana/grafana.pvc.yaml;
...
echo
PROMETHEUS_EXTERNAL_URL='http://monitoring-'"$CLUSTERNAME"'.spectrum.domain.com'
echo -e "Prometheus External URL: ${PROMETHEUS_EXTERNAL_URL}"

#Alertmanager External URL
echo
ALERTMANAGER_EXTERNAL_URL='http://alerting-'"$CLUSTERNAME"'.spectrum.domain.com'
echo -e "Alertmanager External URL: ${ALERTMANAGER_EXTERNAL_URL}"

#Dedicated nodes?
echo
echo -e "${ORANGE}Do you want to use ${WHITE}NodeSelector / Affinity ${ORANGE} to assign monitoring components on dedicated nodes?"
tput sgr0
read -p "Y/N [Y]: " use_dedicated_nodes

#if so, fill out this form...
if [[ $use_dedicated_nodes =~ ^([nN][oO]|[nN])$ ]]; then

  echo -e "${RED}Not using node labels"

  sed -i -e '/AFFINITY/d' manifests/alertmanager/alertmanager.yaml
  ...
  sed -i -e '/AFFINITY/d' manifests/prometheus/prometheus-k8s.yaml
  sed -i -e '/node_label_key/d' manifests/prometheus-operator/prometheus-operator.yaml

else

  echo
  echo -e "${RED}Make sure you label the nodes correctly. The default label is: ${WHITE}${NODE_LABEL_DEFAULT_KEY}=${NODE_LABEL_DEFAULT_VALUE}"
  tput sgr0
  ...
  sed -i -e 's,AFFINITY,'',g' manifests/alertmanager/alertmanager.yaml
  ...
  sed -i -e 's,AFFINITY,'',g' manifests/prometheus/prometheus-k8s.yaml
...
sed -i -e 's/PROMETHEUS_STORAGE_VOLUME_SIZE/'"$PROMETHEUS_STORAGE_VOLUME_SIZE"'/g' manifests/prometheus/self-hosted/persistent-volume.yaml
...
find manifests/grafana -type f -not -path "manifests/grafana/self-hosted/*" -exec kubectl --namespace "$NAMESPACE" apply -f {} \;
...
find manifests/prometheus -type f -not -path "manifests/prometheus/self-hosted/*" -type f ! -name prometheus-k8s-rbac.yaml -exec kubectl --namespace "$NAMESPACE" apply -f {} \;
...
if [[ "$(kubectl get pods -n kube-system | grep scheduler)" ]]; then
  ...
  kubectl apply -f manifests/grafana/self-hosted
else
  echo
